...
class Photos
{
    ...
    public function getPhotos($page=0, $sort='week', $catId=null) {
		...
		$query = "SELECT * FROM pictures INNER JOIN votes ON votes.voteID=pictures.ID WHERE uploadedDate BETWEEN date_sub(now(),INTERVAL 1 $sortBy ) AND now() $catQuery ORDER BY upVotes DESC LIMIT $start , $limit";
